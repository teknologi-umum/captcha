...
func (b *Bot) getUpdates(ctx context.Context, offset, limit int, timeout time.Duration, allowed []string) ([]Update, error) {
	params := map[string]string{
...
func (b *Bot) forwardCopyMany(ctx context.Context, to Recipient, msgs []Editable, key string, opts ...*SendOptions) ([]Message, error) {
	params := map[string]string{
		"chat_id": to.Recipient(),
	}
